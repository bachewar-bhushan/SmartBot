...
GOOGLE_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbyR1a3WvUKxVZQEYidzW4kG3ZovWw-St4tBBphAkQvdSPs7oUAhOdMiZB_655a3cWbn4Q/exec"
while True:
    ...
    mq2_reading = mq2_adc.read_u16() - (97/100 * mq2_adc.read_u16())
    mq3_reading = mq3_adc.read_u16() - (72/100 * mq3_adc.read_u16())
    mq135_reading = mq135_adc.read_u16() - (96/100 * mq135_adc.read_u16())
    print("mq135",mq135_reading)
    print("mq2",mq2_reading)
    print("mq3",mq3_reading)
...
# Usage:
    ...
    data_to_send = {"date": timestamp, "MQ2": mq2_reading, "MQ3": mq3_reading, "MQ135": mq135_reading}
    ...
    try:
        ...
        r = urequests.post("https://script.google.com/macros/s/AKfycbxTCUgCe0LXDMBAdJb3gaVoc2BP6SvN_aDHPEDeR6dsN-MnxA31zG76qhKNdKHUHijVZA/exec?gid=0", json=data_to_send)
        #r.close()
        # Encode data and include it in the URL
        #encoded_data = "&".join([f"{key}={value}" for key, value in data_to_send.items()])
        #url_with_params = f"{GOOGLE_SCRIPT_URL}?{encoded_data}"

        # Send data to Google Spreadsheet
        #response = urequests.get(url_with_params)
        r.close()
        print("Data sent to Google Spreadsheet successfully")
        
    except Exception as e:
        print('Error sending the data:',e)
